...
import { EmployeePickupService } from 'src/app/common/service/employee_pickup_service/employee-pickup.service';
...
import { EmployeePickupModel } from 'src/app/common/model/employee/EmployeePickupModel';
let userDonationData: UserDonationDetails[] = [];
let donationDetails: DonationDetails[] = [];

@Component({
  selector: 'app-employee-pickup-tasks',
  templateUrl: './employee-pickup-tasks.component.html',
  styleUrls: ['./employee-pickup-tasks.component.css']
})
export class EmployeePickupTasksComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    //     getSnackbarProperties('there are no pickup request for your organization', 'red'));
    // }
  ...
  async getDonationDetials(): Promise<any> {
    ...
    let resp = null;
    try {
      resp = await this.employeePickupService.getDonationDetailsByEmpId(this.loginStateService.getLoginState().id);
      ...
      try {
        ...
        if (realStatus) {
          if (realStatus === model.status) {
            resp = await this.employeePickupService.updateEmployeePickupStatus(model.pickupId, status, model.id);
